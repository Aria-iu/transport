...
def find_patches_containing_points(points_csv, patches_csv, patch_size):
    ...
    points_df = pd.read_csv(points_csv, sep='\t')
    # column_names = ['barcode', 'pixel_x', 'pixel_y',  'i', 'j']
    column_names = ['x', 'y', 'new_x', 'new_y', 'pixel_x', 'pixel_y', 'selected']
    ...
    for _, point_row in points_df.iterrows():
        ...
        point_name = str(int(point_row['x'])) + 'x' + str(int(point_row['y']))
...
def batch_process_images_with_points(image_dir, points_dir, patches_dir, patch_size, patch_output_dir):
    ...
    for image_path in image_paths:
        ...
        points_csv = os.path.join(points_dir, f"{image_name}_selection.tsv")
        ...
        # ./ST_Breast/ST-spotfiles\A1_selection.csv
        # ./ST_Breast/patches_csv/patches_512\A1\A1_patches_512.csv
        # ./ST_Breast/cropped_imgs/patches_512\A1\A1
        # print(points_csv)
        # print(patches_csv)
        # print(patch_dir)
...
if __name__ == "__main__":
    root_images = "./ST_Breast/imgs/HE"

    root_points_dir = "./ST_Breast/ST-spotfiles"
    ...
    for sample in os.listdir(root_images):
        ...
        sample = sample[:-4]
        points_dir = os.path.join(root_points_dir)
        patches_dir = os.path.join(root_patches_dir, sample)
        patch_output_dir = os.path.join(root_patch_output_dir, sample)
        # ./ST_Breast/imgs/HE\A1.jpg
        # ./ST_Breast/location\A1
        # ./ST_Breast/patches_csv/patches_512\A1
        # ./ST_Breast/cropped_imgs/patches_512\A1
        # print(images)
        # print(points_dir)
        # print(patches_dir)
